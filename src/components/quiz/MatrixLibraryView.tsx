...
import { ChevronRight, LayoutGrid, Lightbulb, History } from 'lucide-react';
...
export function MatrixLibraryView({ onSelect }: MatrixLibraryViewProps) {
  const categories = [
    ...
    "COM QUEM É O PROBLEMA?",
    "POR QUE O PROBLEMA SE REPETE?",
    "QUAL MENTIRA CONFIÁVEL ESTÁ SENDO CONTADA?",
    "COMO AGIR?"
  ];
    ...
    >
      <div className="mb-12">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h2 className="text-4xl font-serif">Raio-X de Unidade</h2>
        </div>
      ...
      <div className="space-y-16">
        {categories.map((catName) => {
          const categoryMatrices = MATRICES.filter(m => m.category === catName);
          if (categoryMatrices.length === 0) return null;

          return (
            <div key={catName} className="space-y-6">
              <h3 className="text-xl font-bold tracking-[0.2em] text-gold uppercase border-l-2 border-gold/30 pl-4">
                {catName}
              </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {categoryMatrices.map((matrix) => (
                  <Card 
                    key={matrix.id} 
                    className="group hover:border-gold/30 transition-all cursor-pointer relative flex flex-col h-full bg-white/[0.02]" 
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gold/70 leading-relaxed">
              As matrizes têm uma ordem recomendada, como os andares de um prédio. Comece pelo <b>CORPO</b> para silenciar a mente e observar os sinais antes de qualquer conversa profunda. Cada letra do acrônimo representa uma etapa da sua investigação.
            </p>
          </div>
        </div>
      </Card>
